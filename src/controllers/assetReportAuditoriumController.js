...
const addHeader = (worksheet) => {
    ...
    worksheet.getCell('A1').value = 'Laporan Data Aset Ruang Auditorium';
...
const getAllReportAuditoriumAssets = async (req, res) => {
    try {
        ...
        if (auditoriumAssets.length === 0) {
            return res.status(204).json({ message: 'Tidak Ada Aset Yang Ditemukan Di Auditorium' });
        }
        ...
        console.error('Kesalahan pengambilan semua aset auditorium', error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset auditorium' });
    }
};
...
const getReportAuditoriumAssetById = async (req, res) => {
    try {
        const { id } = req.params;
        const auditoriumAsset = await RuangAsetAuditorium.findByPk(id, {
            include: [
                {
                    model: CategoryAsset,
                    as: "asset_category"
        ...
        res.status(200).json({
            ...
            data: auditoriumAsset
        });
    } catch (error) {
        console.error(`Kesalahan pengambilan aset auditorium dengan ID ${id}: `, error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset auditorium' });
    }
};
...
const exportRuangAsetAuditoriumToExcel = async (req, res) => {
    try {
        ...
        const worksheet = workbook.addWorksheet('Data Ruang Aset Auditorium');
...
const searchReportAsset = async (req, res) => {
    try {
        ...
        const filteredAssets = filterAset(auditoriumAssets, filterFields, query);
