...
const addHeader = (worksheet) => {
    ...
    worksheet.getCell('A1').value = 'Laporan Data Aset Ruang Utilitas';
...
const getAllReportUtilitasAssets = async (req, res) => {
    try {
        ...
        if (utilitasAssets.length === 0) {
            return res.status(204).json({ message: 'Tidak Ada Aset Yang Ditemukan Di Utilitas' });
        }

        res.status(200).json({
            message: 'Mendapatkan Semua Laporan Utilitas Sukses',
            data: utilitasAssets
        });
    } catch (error) {
        console.error('Kesalahan pengambilan semua aset utilitas', error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset utilitas' });
    }
};
...
const getReportUtilitasAssetById = async (req, res) => {
    try {
        const { id } = req.params;
        const utilitasAsset = await RuangAsetUtilitas.findByPk(id, {
            include: [
                {
                    model: CategoryAsset,
                    as: "asset_category"
        ...
        res.status(200).json({
            message: `Mendapatkan Laporan Aset utilitas Sukses di ID: ${id}`,
            data: utilitasAsset
        });
    } catch (error) {
        console.error(`Kesalahan pengambilan aset utilitas dengan ID ${id}: `, error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset utilitas' });
    }
};
...
const exportRuangAsetUtilitasToExcel = async (req, res) => {
    try {
        ...
        const worksheet = workbook.addWorksheet('Data Ruang Aset Utilitas');
...
const searchReportAsset = async (req, res) => {
    try {
        ...
        const filteredAssets = filterAset(utilitasAssets, filterFields, query);
