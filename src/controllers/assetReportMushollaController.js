...
const getAllReportMushollaAsset = async (req, res) => {
    try {
        ...
        if (mushollaAssets.length === 0) {
            return res.status(204).json({ message: 'Tidak Ada Aset Yang Ditemukan Di Musholla' });
        }

        res.status(200).json({
            message: 'Mendapatkan Semua Laporan Musholla Sukses',
            data: mushollaAssets
        });
    } catch (error) {
        console.error('Kesalahan pengambilan semua aset musholla', error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset musholla' });
    }
};
...
const getReportMushollaAssetById = async (req, res) => {
    try {
        const { id } = req.params;
        const mushollaAsset = await RuangAsetMusholla.findByPk(id, {
            include: [
                {
                    model: CategoryAsset,
                    as: "asset_category"
        ...
        console.error(`Kesalahan pengambilan aset musholla dengan ID ${id}: `, error);
        res.status(500).json({ message: 'Kesalahan Server Internal, Tidak dapat mengambil aset musholla' });
    }
};
...
const exportRuangAsetMushollaToExcel = async (req, res) => {
    try {
        ...
        const worksheet = workbook.addWorksheet('Data Ruang Aset Musholla');
...
const searchReportAsset = async (req, res) => {
    try {
        ...
        const filteredAssets = filterAset(mushollaAssets, filterFields, query);
